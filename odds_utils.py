...
def get_opposite_outcome_name(market_key: str, all_outcomes: list, outcome: dict):
    ...
    if market_key == "spreads":
        for o in all_outcomes:
            if o["name"] != outcome["name"] and float(o.get("point", 0)) == -float(outcome.get("point", 0)):
                return o["name"]
    ...
    if market_key == "totals":
        ...
        for o in all_outcomes:
            if o["name"] == target and float(o.get("point", 0)) == float(outcome.get("point", 0)):
                return o["name"]
        others = [o["name"] for o in all_outcomes if o["name"] != outcome["name"]]
        return others[0] if others else None
...
def extract_value_rows(events, dk_key="draftkings"):
    ...
    for event in events:
        ...
        for market_key, books_for_market in market_book_map.items():
            ...
            for dk_outcome in dk_outcomes:
                ...
                fair_probs = []
                opposite_name = get_opposite_outcome_name(market_key, dk_outcomes, dk_outcome)
                ...
                for book_key, market in books_for_market.items():
                    ...
                    for o in outcomes:
                        ...
                        same_point = float(o.get("point", 0)) == float(dk_outcome.get("point", 0))
                        ...
                        if opposite_name is not None:
                            ...
                            opp_same_point = (
                                float(o.get("point", 0)) == float(dk_outcome.get("point", 0))
                                if market_key == "totals"
                                else float(o.get("point", 0)) == -float(dk_outcome.get("point", 0))
                                if market_key == "spreads"
                                else True
                            )
    ...
    if not df.empty:
        ...
        df = df.sort_values(["ev_per_dollar", "edge"], ascending=False).reset_index(drop=True)
    return df
